    ...
    async def async_step_ssdp(
        ...
        def _is_supported_device() -> bool:
            ...
            try:
                ...
                return False
            return isinstance(basic_info, dict)
...
class HuaweiLteOptionsFlow(OptionsFlow):
    ...
    async def async_step_init(
        ...
        if user_input is not None:
            ...
            # NEW: a checked "reset_unsupported_endpoints" box clears the
            # persisted list of endpoints detected as unsupported, so they
            # will be probed again on the next reload triggered by the
            # options update.
            if data.pop("reset_unsupported_endpoints", False):
                data[CONF_UNSUPPORTED_KEYS] = []
            return self.async_create_entry(title="", data=data)

        unsupported = sorted(
            self.config_entry.options.get(CONF_UNSUPPORTED_KEYS, [])
        )
        ...
        return self.async_show_form(
            ...
            description_placeholders={
                "sample_ip": "http://192.168.X.1",
                "unsupported_keys": ", ".join(unsupported) or "(none)",
            },
        )
